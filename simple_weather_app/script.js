...
{            
    ...
    if(response.status != 200){
        ...
        let CricketArray = response.data.cricket
        let FootballArray = response.data.football

        for(Key in CricketArray)
        {
            let html = "<tr><td>"+CricketArray[Key].match+"</td><td>"+CricketArray[Key].tournament+"</td><td>"+CricketArray[Key].stadium+"</td><td>"+CricketArray[Key].start+"</td></tr>"
            document.getElementById('cricket').insertAdjacentHTML('afterend', '<tr><td>'+html+'</td</tr>')
            // console.log(Array[Key].condition.text)
        }

        for(Key in FootballArray)
        {
            let html = "<tr><td>"+FootballArray[Key].match+"</td><td>"+FootballArray[Key].tournament+"</td><td>"+FootballArray[Key].stadium+"</td><td>"+FootballArray[Key].start+"</td></tr>"
            document.getElementById('football').insertAdjacentHTML('afterend', '<tr><td>'+html+'</td</tr>')
            // console.log(Array[Key].condition.text)
        }
